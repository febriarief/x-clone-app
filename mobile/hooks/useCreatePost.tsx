...
export const useCreatePost = () => {
	...
	const handleImagePicker = async (useCamera: boolean = false) => {
		...
		if (permissionResult.status !== 'granted') {
			...
			Alert.alert('Permission needed', `Please grant permission to access yout ${source}`);
			return;
		}

		const pickerOptions = {
			allowedEditing: true,
			aspect: [16, 19] as [number, number],
			quality: 0.8
		};
	...
	const createPost = () => {
		if (!content.trim() && !selectedImage) {
			Alert.alert('Empty Post', 'Please write somthing or add an image before posting!');
			return;	
		}
